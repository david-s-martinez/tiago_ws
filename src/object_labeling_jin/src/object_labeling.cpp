...
bool ObjectLabeling::labelObjects(CloudPtr& input, CloudPtrl& output)
{
  ...
  {
    // cluster.reset(new PointCloud);
    // for (size_t pit = 0; pit < it->indices.size(); ++pit)
    //   cluster->push_back((*input)[pit]);
   
    // float sum_x, sum_y, sum_z = 0;
    // for (size_t i = 0; i < cluster->size(); i++)
    // { 
    //   sum_x = sum_x + cluster->points[i].x;
    //   sum_y = sum_y + cluster->points[i].y;
    //   sum_z = sum_z + cluster->points[i].z;
    // }
    // float centroid_x = sum_x / cluster->size();
    // float centroid_y = sum_y / cluster->size();
    // float centroid_z = sum_z / cluster->size();
  ...
  ROS_WARN_STREAM("bbx number: " << detections_.size());
  ...
  for(size_t i = 0; i < assigned_classes.size(); ++i)
  {
    ...
    ROS_WARN_STREAM("assgined class: "<<assigned_classes[i]);
  }
  ROS_WARN_STREAM("marker number: "<<text_markers_.markers.size());
